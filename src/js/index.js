...
login.addEventListener('click', () => {
  ...
  spanUsuario = document.getElementById('spanNomeUsuarioPerfil');

  if (spanUsuario.textContent != "") {
    divLogin.classList.replace('d-none', 'd-block');
  } else {
    divLogin.classList.replace('d-block', 'd-none');
  }
...
//Buscar por leitor de codigo de barras tela pequena
const eventobotaosmart = document.getElementById('eventobotaocodigo');

eventobotaosmart.addEventListener('click', () => {
  // Inicialize o Quagga
  const config = {
    inputStream: {
      name: "Live",
  ...
  Quagga.init(config, () => {
    // Quando o Quagga estiver inicializado, inscreva-se no evento 'detected'
    Quagga.onDetected((result) => {
      var code = result.codeResult.code;
      var acao = "buscarPorCod";
      Quagga.stop(); // Pare a câmera
      $('#modal-camera').modal('hide');
      ajaxListarPromocao(acao, code);
    });

    // Inicie o Quagga após se inscrever no evento
    Quagga.start();
  });
});

//buscar produto por descrição tela pequena
document.getElementById("buscarPromoPorDescPeq").addEventListener("click", function () {
  var code = document.getElementById("inputBuscarPromoPorDescPeq").value;
  var acao = "buscarPorDesc";
    ...
    success: function (retorno) {
      if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(function (position) {
          ...
          function calcularDistanciaPromise(origem, destino) {
          ...
          // Promessas para calcular distâncias
          const promises = retorno.map(item => {
            const destino = `${item.latitude},${item.longitude}`;
            return calcularDistanciaPromise(`${userLat},${userLng}`, destino);
          });
            ...
            .then(distancias => {
              ...
              if (retorno.length > 0) {
                conteudoHTML = `
                  ...
                  <div class="row justify-content-center mb-3" id="rowPromoListar">
                    ${cardListarPromo(retorno, distancias)}
                  </div>
                `
      ...
      function cardListarPromo(retorno, distancias) {
        return retorno.map(item => `
        <a href="detalheProduto.html?codigo=${item.codigo}" style="color:black;"><div class="col-lg-4 col-md-3 d-flex justify-content-center mt-3 mx-1 cardIndexListar">
          <div class="container">
            <div class="row mt-2">
              <div class="col d-flex justify-content-center" style="height:50px">
                <span id="spanSuperListar">
                  ${item.supermercado}
                </span>
                <span id="codProd" style="display:none;">
                  ${item.codigo}
                </span>
              </div>
            </div>
            <div class="row">
              <img class="mx-auto d-block img-fluid rounded" src="${item.imagem}" style="max-height:100px; max-width:100px">
            </div>
            <div class="row">
            ...
            <div class="row mt-3">
              ...
              <div class="col d-flex justify-content-end">
                ...
                <span>
                  ${distancias[retorno.indexOf(item)]}
                </span>
              </div>
            </div>
          </div>
      ...
      function buscaDiaHora(horas) {
        if (horas >= 24) {
          const dia = horas / 24;
          return dia === 1 ? `há ${parseInt(dia)} dia` : `há ${parseInt(dia)} dias`;
        } else if (horas !== 0) {
          ...
          Promise.all(promises)
            .then(distancias => {
              for (var i = 0; i < retorno.length; i++) {
                ...
                spanFilho3.innerHTML = " R$ " + (retorno[i].valor).replace('.',',');
...
inputSupermercado.addEventListener("input", function () {
  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      const request = {
        location: new google.maps.LatLng(userLat, userLng), // Coordenadas de referência
        radius: 500,
        type: ['supermarket'],
        query: searchTerm,
      };
...
document.getElementById("btnCadPromo").addEventListener("click", function () {
  ...
  if (valor.value == "" || valor.value == 0.00) {
    ...
    var u_valor = (valor.value).replace(',','.');
  }
  ...
  if(document.getElementById("divInput-codigodig").classList.contains("invalidoCodigo")){
     u_invalido = true;
  }else{
    u_invalido = false;
  }
  if (u_valor != null && u_codigoBarCode != null && (u_dataInc != null || (u_dataInc == null && u_naoInformar == true)) && u_invalido == false) {
...
document.getElementById("lerCodeBar").addEventListener("click", function () {
  ...
  document.getElementById("input-codigodig").value = "";
  document.getElementById("imgCad").src = "";
  document.getElementById("divInput-codigodig").classList.remove("invalidoCodigo");
  document.getElementById("labelCodigoDig").style.display = "none";

  Quagga.onDetected((result) => {
    var code = result.codeResult.code;
    document.getElementById("input-codigodig").value = code;
  const apiUrl = "https://api.cosmos.bluesoft.com.br/gtins/" + code + ".json";
  ...
  // solicitação GET para a API Cosmos
  axios.get(apiUrl, axiosConfig)
    .then(response => {
      document.getElementById("input-descricao").value = response.data.description;
      var imgProduto = document.getElementById("imgCad");
    ...
    .catch(error => {
      ...
      if (!document.getElementById("input-codigodig").value == "") {
        const divCodigoDig = document.getElementById("divInput-codigodig");
        if (!divCodigoDig.classList.contains("invalidoCodigo")) {
          const labelCodigodig = document.getElementById("labelCodigoDig");
          labelCodigodig.style.color = "red";
          labelCodigodig.innerHTML = "Código de barra incorreto. Realize a leitura novamente.";
          divCodigoDig.classList.add("invalidoCodigo");
          labelCodigodig.style.display = "block";
  ...
  $.ajax({
    ...
    success: function (retorno) {
      if (retorno['resultado'] == "logado") {
        ...
        //spanOla.classList.add("logado");
        spanOla.setAttribute("id", "spanUsuario");
        spanOla.classList.add()
        ...
        const spanNomeUsuarioPerfil = document.getElementById("spanNomeUsuarioPerfil");
        spanNomeUsuarioPerfil.innerHTML = spanOla.textContent;
        spanNomeUsuarioPerfil.style.display = "block";
        document.getElementById("Entrar").style.display = "none";
        document.getElementById("sair").style.display = "block";
        document.getElementById("btnMinhasPromo").style.display = "block";
      } else {
...
document.getElementById('btnInserirPromoPeq').addEventListener("click", function () {
  u_acao = "verificarAcesso";
  $.ajax({
    url: '../precofacil/src/php/cadastrocliente.php',
    method: 'GET',
...
document.getElementById("valorPromo").addEventListener("blur", function () {
  $(document).ready(function () {
    $('#valorPromo').mask('000.000.000.000,00', { reverse: true });
  });
...
document.getElementById('fecharPerfil').addEventListener('click', function () {
